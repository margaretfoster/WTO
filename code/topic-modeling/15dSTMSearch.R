...
## words included at the 1% threshold
## produces the "frustration" theme
...
out <- prepDocuments(processed$documents,
                     processed$vocab,
                     processed$meta,
                     lower.thresh=el*.001, ## word must be in at least 1% of docs
...
save(search.wide,
     file="WTOSearchKwide_seed1.Rdata")
...
sink(file="topicwords.k45.txt")
summary(inverse.trump.k45)
sink()



inverse.trump.k50 <- stm(documents=out$documents,
                 data=out$meta,
                 vocab=out$vocab,
                 seed=689,
                 K=50,
...
sink(file="topicwords.inversetrump.k50.txt")
summary(inverse.trump.k50)
sink()

save.image(file="WTO_STM_Sweep_CustomStopwords.Rdata")

inverseTrump.ee <-estimateEffect
