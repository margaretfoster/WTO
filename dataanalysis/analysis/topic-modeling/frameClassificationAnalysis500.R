...
## 182
## temporary drop and come back with the PID
...
tagged$X.y <- NULL
tagged$X.x <- NULL
tagged$PID <- as.numeric(tagged$PID)
...
table(tagged$Frame) ## 84 reciprocator; 179 redist;
## 224 unknown
...
## Plots:
pdf(file="ROCplot500-naivebayes.pdf")
autoplot(nb.roc) +
    labs(title = "Naive Bayes"
         )
dev.off()

pdf(file="ROCplot500-svm.pdf")
autoplot(svm.roc) +
    labs(title = "SVM"
         )
dev.off()

pdf(file="ROCplot500-randomforest.pdf")
autoplot(rf.roc) +
    labs(subtitle = "Random Forest"
         )
dev.off()

pdf(file="ROCplot500-glmlasso.pdf")
autoplot(glm.roc) +
...
write.csv(tst.out, "wto-hand-class500.csv")
...
pdf(file="ROCPlot500-delegatelists.pdf")
autoplot(hand.roc) +
...
gg
ggsave(gg, file="classificationAllMods500.pdf")
...
tagged.pids <- as.numeric(tagged$PID)
tagged.pids <- na.omit(tagged.pids)

length(tagged.pids) ##487, one need to be repaired

tagged.pids

untagged <- meta[!(meta$PID %in% as.numeric(tagged.pids)),]

dim(untagged) ##8367 x 16
...
dim(wto.meta.aug) ## 8367 x 20
colnames(wto.meta.aug)
