...
load(paste0(dataPathDesktop,
            "programsSubSetstmYearStemmedFac.Rdata"))

## Verify the specification:

ls()

mod.programs.themefaction$settings$call

faction.model ## ~s(year)*income
...
ggdat <- stminsights::get_effects(
    estimates=prep.programs.themefaction,
    variable="year",
    type="continuous",
    moderator="faction",
    modval=c("US-EU-Can")) %>%
...
ggsave(file="ProgramsFac.png")

## Technical Assistance Seems in the overall graph:
ggdat %>%
   filter(topic == 8) %>%
    ##filter(moderator %in% ilevels) %>%
   ## filter(moderator ==  "LIC") %>% ## LIC blows up for all topics
