...
prep.out10.themefaction <- estimateEffect(c(1:10) ~s(meetingno)+ faction,
...
prep.out10.themecat <- estimateEffect(c(1:10) ~s(meetingno)+ cat,
                         mod.out10.themecat,
                          metadata=out$meta,
                          documents=out$documents,
                          uncertainty=c("Global"))
...
save.image(file="ProgramsSubSetstm.Rdata")
