...
#####################
## Describe model:
## Can't use these b/c we use the K=2 results to identify the categories!
...
prep.2.fac <- prep.2
## ## Faction Model:
## mod.out.2.fac <- stm(documents=out$documents,
##                      vocab=out$vocab,
##                      data=out$meta,
##                      K=2, ## 
...
## prep.2.fac <- estimateEffect(c(1:2) ~s(year) + faction,
##                              mod.out.2.fac,
##                              metadata=out$meta,
##                              documents=out$documents,
##                              uncertainty=c("Global"))

save.image(file="twoTopicsAndSubSets-NoAdminSubset_CatFac.Rdata")
