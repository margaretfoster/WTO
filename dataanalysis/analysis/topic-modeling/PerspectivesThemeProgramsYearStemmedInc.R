...
mod.prog.themeinc <- stm(documents=out$documents,
                        vocab=out$vocab,
                        data=out$meta,
                        K=10, ## 
...
prep.prog.themeinc <- estimateEffect(c(1:10) ~s(year)+ income_level_iso3c,
                                        mod.prog.themeinc,
                                        metadata=out$meta,
                                        documents=out$documents,
                                        uncertainty=c("Global"))

summary(mod.prog.themeinc)

save.image(file="ProgramsSubSetstmYearStemmedInc.Rdata")
